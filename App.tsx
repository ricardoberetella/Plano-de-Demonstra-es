  ...
  <header className="bg-[#004B95] text-white shadow-xl sticky top-0 z-50 print:hidden">
    <div className="max-w-7xl mx-auto px-4 h-20 flex justify-between items-center">
      ...
      <div className="flex gap-2">
        <button onClick={onHome} className="bg-white/10 px-4 py-2 rounded-lg font-bold text-xs uppercase">Início</button>
...
const DetailView: React.FC<{ op: OperationPlan }> = ({ op }) => (
  <div className="pb-10 space-y-6 max-w-5xl mx-auto">
    ...
    <div className="bg-white border-2 border-black rounded-xl overflow-hidden shadow-md">
      <div className="flex border-b-2 border-black bg-slate-50">
        ...
        <div className="w-2/3 p-4 text-center flex flex-col justify-center">
          ...
          <p className="text-xs font-bold uppercase text-slate-500">Operação Nº {op.reference}</p>
        </div>
      </div>
      <div className="p-6">
    ...
    {/* Tabela de Passos e Pontos-Chave */}
    <div className="bg-white border-2 border-black rounded-xl overflow-hidden shadow-lg">
      <div className="grid grid-cols-2 bg-black text-white text-[10px] font-black uppercase p-3 text-center tracking-widest">
        ...
        {op.steps.map((step, i) => (
          <div key={i} className="grid grid-cols-2 min-h-[80px]">
            <div className="p-4 border-r-2 border-black flex gap-3">
              ...
              <p className="text-[11px] font-black uppercase leading-tight pt-1">{step.task}</p>
            </div>
            <div className="p-4 bg-slate-50 flex flex-col justify-center space-y-2">
              {step.keyPoints.map((point, pi) => (
                <div key={pi} className="flex gap-2 text-[10px] font-bold text-slate-800 uppercase leading-tight italic">
    ...
    {/* SEÇÃO RECUPERADA: PERGUNTAS DE VERIFICAÇÃO */}
    <div className="bg-[#f1f5f9] border-2 border-black rounded-xl p-6 shadow-md">
      <h3 className="text-sm font-black uppercase text-[#004B95] mb-4 flex items-center gap-2">
        <span className="bg-[#004B95] text-white p-1 rounded">?</span>
        Perguntas de Verificação
      </h3>
      <div className="space-y-4">
        {op.verificationQuestions && op.verificationQuestions.length > 0 ? (
          op.verificationQuestions.map((q, index) => (
            <div key={index} className="bg-white p-4 rounded-lg border-l-4 border-[#E30613] shadow-sm">
              <p className="text-xs font-black uppercase text-slate-700 leading-relaxed">
                {index + 1}. {q}
              </p>
            </div>
          ))
        ) : (
          <p className="text-xs italic text-slate-500 font-bold uppercase">Nenhuma pergunta cadastrada para esta operação.</p>
        )}
      </div>
    </div>
  </div>
      ...
      <div className="min-h-screen bg-[#0a192f] flex items-center justify-center p-4 font-sans">
        <div className="w-full max-w-sm bg-white rounded-[30px] overflow-hidden shadow-2xl">
          ...
          <form className="p-8 space-y-4" onSubmit={(e) => { e.preventDefault(); if(password === "ianes662") setIsAuthenticated(true); }}>
            ...
            />
            <button className="w-full bg-[#004B95] text-white font-black py-4 rounded-full uppercase text-sm tracking-widest hover:brightness-110 transition-all">
              Acessar
            </button>
          </form>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {!selectedOp ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {OPERATIONS.map(op => (
              <div key={op.id} onClick={() => setSelectedOp(op)} className="bg-white border-2 border-slate-200 rounded-2xl p-6 shadow-sm cursor-pointer hover:border-[#004B95] transition-all group">
                <span className="text-[9px] font-black px-2 py-1 rounded bg-slate-100 text-slate-500 uppercase">{op.category}</span>
                <h3 className="text-lg font-black uppercase mt-3 mb-4 group-hover:text-[#004B95]">{op.name}</h3>
                <div className="pt-3 border-t flex justify-between items-center text-[#E30613] font-black text-[10px] uppercase italic">
                  <span>Ref: {op.reference}</span>
                  <span className="bg-[#E30613] text-white px-3 py-1 rounded-full not-italic">Abrir</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            <button onClick={() => setSelectedOp(null)} className="mb-6 font-black text-[#004B95] uppercase flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-200">
              ← Voltar
            </button>
